...
intValue = function(validationValue){
	var RegExPattern = /^(?:\+|-)?\d+$/;
	if (validationValue.match(RegExPattern) && (validationValue!='')) {return true;}
	else {return false;}
}
reqValue = function(validationValue){
	if (validationValue.replace(/^(\s|\&nbsp;)*|(\s|\&nbsp;)*$/g,"").length==0) {return false;}
	else {return true;}
}
minLength = function(validationValue,minLength){
	if (validationValue.replace(/^(\s|\&nbsp;)*|(\s|\&nbsp;)*$/g,"").length<minLength) {return false;}
	else {return true;}
}
maxLenght = function(validationValue,maxLength){
	if (validationValue.replace(/^(\s|\&nbsp;)*|(\s|\&nbsp;)*$/g,"").length>maxLength) {return false;}
	else {return true;}
}
